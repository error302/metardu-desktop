...
export interface CsvParseResult {
  points: SurveyPoint[];
  errors: { row: number; line: string; error: string }[];
}
...
export function parseCsvPoints(csvContent: string): SurveyPoint[] {
  ...
  if (pointCol === undefined || eastingCol === undefined || northingCol === undefined) {
    throw new Error(
      `CSV header is missing required columns. Need point_number (or point/pt/pid/id), easting (or east/x/e), northing (or north/y/n). Got: ${header.join(', ')}`,
    );
  }
