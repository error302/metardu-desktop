...
> = {
  ...
  async export(input, options): Promise<LandxmlOutput> {
    ...
    const projectMetadata = options.projectMetadata as Record<string, unknown> | undefined;
    const projectName = (projectMetadata?.projectName as string) ?? "Survey";
    const surveyorName = projectMetadata?.surveyorName as string | undefined;
    const srid = (options as any).srid as number | undefined;
