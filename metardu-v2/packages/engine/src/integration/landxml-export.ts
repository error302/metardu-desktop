...
> = {
  ...
  fileExtension: "xml",
  description: "Exports survey data to LandXML 1.2 format",
  ...
  async export(input, options): Promise<LandxmlOutput> {
    ...
    if ("allBeacons" in obj && Array.isArray(obj.allBeacons)) {
      ...
      xml = exportCadastralXml(beacons, countryCode, projectName, surveyorName, options.targetSrid);
    }
    // Topographic output (has tin + contours)
      ...
      xml = exportTopographicXml(points, contours, projectName, options.targetSrid);
    }
    else {
